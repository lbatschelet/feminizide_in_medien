...
These terms are the basis for the queries. We define them centrally to keep usage consistent across analysis and API calls.
...
Build a quanteda corpus and a trimmed DFM from the cleaned articles. Use the checkpoint load block to resume without reprocessing.
...
```{r second-pass-query, cache=FALSE}
...
```{r second-pass-prep, cache=FALSE}
...
cat("=== SECOND-PASS: ARTICLES PER YEAR ===\n")
print(articles_second_by_year)

# Top sources
cat("\n=== SECOND-PASS: TOP MEDIA ===\n")
top_media_second <- media_data_second %>%
  count(medium_name, sort = TRUE) %>%
  head(15)
print(top_media_second)
...
cat("\n=== SECOND-PASS: KEY TERMS BY PERIOD (LOG2 RATIO) ===\n")
print(top_contrast %>% select(term, log2_ratio, direction))
```

```{r checkpoint-save-secondpass-analysis}
# Checkpoint save (Second-pass analysis outputs)
save_checkpoint(top_contrast, "secondpass_key_terms")
```

## Second-pass semantic expansion (iteration 2)
...
```{r second-pass-word2vec-training}
...
save_checkpoint(word2vec_models_secondpass, "word2vec_models_secondpass")
```

```{r checkpoint-load-word2vec-secondpass}
# Checkpoint load (Second-pass Word2Vec)
load_checkpoint("word2vec_models_secondpass", "word2vec_models_secondpass")
```

```{r second-pass-word2vec-neighbors}
...
neighbors_secondpass <- dplyr::bind_rows(lapply(names(word2vec_models_secondpass), function(w) {
  model <- word2vec_models_secondpass[[w]]
  df <- collect_neighbors(model, seed_terms_secondpass, top_n = 30)
  df$window <- w
  df
}))
...
cat("\n=== SECOND-PASS: CANDIDATE TERMS (ITERATION 2) ===\n")
print(candidate_terms_secondpass)
```