...
```{r}
#| label: feminizid-data-collection
#| cache: false
...
#| fig-cap: "Coverage overview and outlet distribution."
#| fig-subcap: ["Articles per year", "Top outlets", "Monthly trend (last five years)"]
...
#| fig-cap: "Keyword frequency in headlines (counts across corpus)."
...
```{r}
#| label: time-trends
#| fig-cap: "Keyword trends over time."
#| fig-subcap: ["Absolute counts per year", "Heatmap of keyword intensity by year"]
...
```{r}
#| label: target-term-trends
#| fig-cap: "Framing groups over time (frequency per million words)."
...
```{r}
#| label: visualization
#| fig-cap: "Lexical visualization and frequency normalization."
#| fig-subcap: ["Top terms wordcloud", "Top terms over time (absolute)", "Top terms over time (per 1000 articles)", "Article distribution over time"]
...
```{r}
#| label: checkpoint-load-word2vec
#| echo: false
#| results: hide
#| message: false
#| warning: false
...
```{r}
#| label: word2vec-visualization
#| fig-cap: "Frame-shift heatmaps and stability summary."
...
```{r}
#| label: word2vec-local-embedding
#| fig-cap: "Local 3D embedding of target terms and neighbors (UMAP)."
set.seed(123)
...
We set the start date to 1990 for two reasons. First, coverage and metadata quality improve markedly from the early to mid‑1990s, so earlier years are both sparse and less reliable. Second, restricting the window keeps computation feasible; extending further back would substantially increase runtime and storage without proportional analytical gain. The 1990 threshold therefore balances data availability with practical constraints.
...
```{r}
#| label: second-pass-filter-rubrics
#| cache.lazy: false
...
```{r}
#| label: checkpoint-save-secondpass-analysis
#| cache.lazy: false
#| echo: false
#| results: hide
#| message: false
...
```{r}
#| label: second-pass-word2vec-setup
#| cache.lazy: false
...
```{r}
#| label: second-pass-word2vec-training
#| cache: false
...
```{r}
#| label: checkpoint-load-word2vec-secondpass
#| cache.lazy: false
#| echo: false
#| results: hide
#| message: false
...
```{r}
#| label: second-pass-word2vec-neighbors
#| cache.lazy: false
...
```{r}
#| label: second-pass-word2vec-3d
#| cache.lazy: false
#| fig-cap: "Aligned 3D timeline of target terms and neighbors across windows."
...
The contrast between euphemistic and explicit naming becomes particularly visible when aggregated into a simplified time series. @fig-framing-shift-summary contrasts normalized frequencies of selected euphemistic framings with explicit naming practices per million words over time.
...
```{r}
#| label: fig-framing-shift-summary
...
In her more recent work, @dignazio_counting_feminicide sharpens this point by focusing explicitly on feminicide as a problem of enumeration and recognition: counting is not merely a technical operation, but a political struggle over whose deaths become publicly nameable, recordable, and therefore grievable in institutional terms. Read through this lens, our corpus is doubly filtered: first by journalistic selection and framing, and second by our own retrieval and modeling choices. The analytical object is therefore not feminicide “in Switzerland,” but feminicide as it becomes discursively and infrastructurally countable within a specific media archive.

This has direct implications for interpretation. Killings framed without gender markers, relational descriptors, or recognizable violence terms may never enter the analytical space. In this sense, the corpus captures discursive recognition rather than empirical prevalence. The silences in the data are therefore analytically meaningful, even though they cannot be directly quantified. More importantly, those silences are not random; they are patterned outcomes of editorial practices, social norms, and institutional thresholds for relevance—conditions that determine which forms of violence become publicly narratable and which remain backgrounded, euphemized, or absent.

A further limitation concerns event-driven distortions. High-profile cases can dominate coverage and skew term distributions. In our dataset, repeated references to *Hochseilartist* trace back to extensive reporting on the Freddy-Nock case, which ultimately ended in an acquittal [@noauthor_toxische_2020]. Such cases illustrate how individual media events can introduce semantic noise that is unrelated to feminicide discourse as such, and they highlight a broader methodological tension: large-scale text analysis can easily become sensitive to notoriety and media logics of attention, even when the substantive research interest is a structurally patterned form of gendered violence.
