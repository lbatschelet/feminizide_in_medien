...
if (file.exists("local.env")) {
  ...
  message("⚠️ local.env nicht gefunden - verwende System-Environment")
}
...
```{r data-preparation}
# Daten laden (suche nach vorhandenen Dateien)
if (!exists("media_data")) {
  # Prüfe zuerst nach .rds Dateien
  rds_files <- list.files("data", pattern = "feminizid_raw_data_.*\\.rds$", full.names = TRUE)
  if (length(rds_files) > 0) {
    # Neueste .rds Datei verwenden
    latest_rds <- rds_files[order(file.mtime(rds_files), decreasing = TRUE)][1]
    message("Lade gespeicherte Rohdaten: ", basename(latest_rds))
    media_data <- readRDS(latest_rds)
    message("✓ Rohdaten geladen: ", nrow(media_data), " Artikel")
  } else {
    # Fallback: suche nach .tsv Dateien
    ...
    if (length(tsv_files) > 0) {
      ...
      for (file in tsv_files) {
        ...
        if (grepl("\\.xz$", file)) {
          # Komprimierte Datei
          data_chunk <- read_tsv(file, locale = locale(encoding = "UTF-8"))
        } else {
          # Normale TSV
          data_chunk <- read_tsv(file, locale = locale(encoding = "UTF-8"))
        }
      ...
      media_data <- bind_rows(all_data) %>%
        distinct(id, .keep_all = TRUE)
      message("✓ ", length(tsv_files), " Dateien kombiniert: ", nrow(media_data), " Artikel")
      
      # Als .rds speichern für nächstes Mal
      current_rds <- paste0("data/feminizid_raw_data_", format(Sys.Date(), "%Y%m%d"), ".rds")
      message("Speichere kombinierte Daten als: ", basename(current_rds))
      saveRDS(media_data, current_rds, compress = "xz")
    } else {
      message("⚠️ Keine Datendateien gefunden - führen Sie API-Call aus")
    }
  }
}
```
...
if (exists("feminizid_corpus")) {
  ...
  # Multi-Word-Expressions (Wortpaare) definieren
  multiword_patterns <- list(
    # Gewalt-Begriffe
    c("häusliche", "gewalt"),
    c("partnerschafts", "gewalt"), 
    c("partner", "gewalt"),
    c("beziehungs", "gewalt"),
    c("sexuelle", "gewalt"),
    c("körperliche", "gewalt"),
    c("psychische", "gewalt"),
    ...
    # Justiz-Begriffe
    c("häuslicher", "gewalt"),  # Alternative Schreibweise
    c("opfer", "schutz"),
    c("gewalt", "schutz")
  )
  ...
  cat("=== MULTI-WORD-EXPRESSIONS ===\n")
  cat("Definierte Wortpaare:", length(multiword_patterns), "\n")
  cat("Beispiele: häusliche_gewalt, partnerschafts_gewalt, tötung_frau\n")
...
if (exists("feminizid_dfm_trimmed")) {
  ...
  if (exists("feminizid_corpus")) {
    ...
    if (summary_stats$ratio_max_min > 5) {
      cat("⚠️ Starke Verzerrung erkannt - normalisierte Darstellung empfohlen!\n")
    }
  }
}
```
