...
relation_terms <- c(
  "Familiendrama", "Familientragödie", "Beziehungstat", "Ehestreit",
  "häusliche Gewalt", "Partnerschaftsgewalt", "Gewalt gegen Frauen",
  "Partnergewalt", "Beziehungsgewalt"
)
...
action_terms_wildcards <- add_wildcards_single(action_terms)
relation_terms_wildcards <- add_wildcards_single(relation_terms)

action_terms_v1 <- unique(c(action_terms, action_terms_wildcards))
relation_terms_v1 <- unique(c(relation_terms, relation_terms_wildcards))
```
...
We start with the original keyword list (broad OR query).
...
Split the suggested terms into action/context and relation terms, then
re-run the API call with a stricter AND query (from 1990).
...
action_terms_extra <- c(
  ...
  "geschlagen", "misshandelt", "stranguliert",
  "kopfschuss", "küchenmesser",
  "mord", "mordopfer", "gewalt", "gewalttätig",
  "stalking", "peiniger", "eifersucht"
)

relation_terms_extra <- c(
  "beziehung", "ehepartner", "exmann", "freundin", "gattin",
  "geschieden", "intimpartner", "lebensgefährten",
  "lebensgemeinschaft", "partnerinnen", "partnerschaft",
  "schwägerin", "tochter", "trennung"
)
...
action_terms_extra_wildcards <- paste0(action_terms_extra, "*")
relation_terms_extra_wildcards <- paste0(relation_terms_extra, "*")

action_terms_v2 <- unique(c(action_terms_v1, action_terms_extra, action_terms_extra_wildcards))
relation_terms_v2 <- unique(c(relation_terms_v1, relation_terms_extra, relation_terms_extra_wildcards))

second_pass_result <- execute_swissdox_workflow(
  client = client,
  query = create_swissdox_query(
    content_groups = list(
      list(operator = "OR", terms = action_terms_v2),
      list(operator = "OR", terms = relation_terms_v2)
    ),
...
```{r second-pass-prep}
media_data_second <- second_pass_result$data
save_checkpoint(media_data_second, "feminizid_raw_data_secondpass")

media_data_second$pubtime <- as.POSIXct(media_data_second$pubtime)
media_data_second$pub_date <- as.Date(media_data_second$pubtime)
media_data_second$year <- as.numeric(format(media_data_second$pub_date, "%Y"))
