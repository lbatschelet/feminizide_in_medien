...
save_checkpoint <- function(obj, prefix, dir = "data", compress = "xz") {
  ...
  message("Gespeichert: ", basename(file))
  file
}
...
# Keyword list (central definition)
...
if (!exists("media_data")) {
  if (!load_checkpoint("feminizid_raw_data", "media_data")) {
...
stopifnot(exists("feminizid_corpus"))
  ...
  # ERST Stemming (reduziert alle Flexionsformen auf Grundform)
  ...
  simplified_patterns <- list(
    # Gewalt-Begriffe (nur Grundformen nach Stemming)
    c("hauslich", "gewalt"),      # h\u00e4usliche/h\u00e4uslicher \u2192 h\u00e4usl
    c("partnerschaft", "gewalt"), 
    c("partner", "gewalt"),
    c("bezieh", "gewalt"),     # beziehungs \u2192 bezieh
    c("sexuell", "gewalt"),    # sexuelle/sexueller \u2192 sexuell
    c("k\u00f6rperl", "gewalt"),    # k\u00f6rperliche/k\u00f6rperlicher \u2192 k\u00f6rperl
    c("psychisch", "gewalt"),
    
    # Feminizid-Begriffe
    c("t\u00f6t", "frau"),          # t\u00f6tung \u2192 t\u00f6t
    c("mord", "frau"),
    c("gewalt", "frau"),
    
    # Familien-Begriffe
    c("famili", "drama"),      # familien \u2192 famili
    c("famili", "trag\u00f6di"),    # trag\u00f6die \u2192 trag\u00f6di
    c("bezieh", "tat"),        # beziehungs \u2192 bezieh
    c("eh", "streit"),         # ehe \u2192 eh
    
    # Justiz-Begriffe
    c("opf", "schutz"),        # opfer \u2192 opf
    c("gewalt", "schutz")
  )
...
stopifnot(exists("feminizid_corpus"))
  ...
  for (window_name in names(time_windows)) {
    ...
    # Embeddings f\u00fcr Zielw\u00f6rter extrahieren
    available_words <- intersect(target_words, rownames(as.matrix(w2v_model)))
    if (length(available_words) > 0) {
      ...
      cat("\u2713 Embeddings f\u00fcr", length(available_words), "Zielw\u00f6rter extrahiert\n")
    } else {
      cat("WARN: Keine Zielw\u00f6rter in", window_name, "gefunden\n")
    }
  }
...
if (length(word2vec_models) >= 2) {
  ...
  for (target_word in target_words) {
    ...
    for (window_name in names(word2vec_models)) {
      ...
      if (target_word %in% rownames(as.matrix(model))) {
        
        # Top 10 \u00e4hnlichste W\u00f6rter finden
        similar_words <- predict(model, target_word, type = "nearest", top_n = 10)
  ...
  if (length(frame_shift_results) > 0) {
    ...
    # Beispiel: Top-Ver\u00e4nderungen f\u00fcr "hauslich_gewalt"
    if ("hauslich_gewalt" %in% names(frame_shift_results)) {
      cat("\n=== BEISPIEL: h\u00e4usliche_gewalt Frame-Shifts ===\n")
      hauslich_shifts <- frame_shift_results[["hauslich_gewalt"]]
      
      # Top 3 Nachbarn pro Zeitfenster
      top_neighbors <- hauslich_shifts %>%
        filter(rank <= 3) %>%
