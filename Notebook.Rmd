...
if (exists("word2vec_models")) {
  if (!requireNamespace("umap", quietly = TRUE)) {
    ...
    # Use the latest time window (based on end year in the label)
    window_end_year <- function(x) as.integer(sub(".*-(\\d{4})$", "\\1", x))
    viz_window <- names(word2vec_models)[which.max(window_end_year(names(word2vec_models)))]
